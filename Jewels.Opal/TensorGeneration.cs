...
public static partial class Operations
{
    ...
    public static Tensor<float[,]> HeMatrix(int rows, int columns)
    {
        ...
        return GenerateMatrix((_, _) => NextGaussian() * scale, rows, columns);
    }
    ...
    public static Tensor<float[]> HeVector(int size, int fanIn)
    {
        ...
        return GenerateVector(_ => NextGaussian() * scale, size);
    }
    ...
    public static float NextSingle(float min = -1, float max = 1) => random.NextSingle() * (max - min) + min;
}
