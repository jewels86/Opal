...
namespace Testing
{
	internal class Program
	{
		...
		{
			EmbeddingsModule<string> embeddings = new(16, 128, 32, 0.5);
			...
			Console.WriteLine($"sim: {embeddings.QuickSimilarity(hash1, hash2)}");
			Console.WriteLine($"sim: {embeddings.CosineSimilarity(e1.Vector, e2.Vector)}");
			embeddings.Associate(e1, e2);
			Console.WriteLine($"sim: {embeddings.QuickSimilarity(e1, e2)}");
			Console.WriteLine($"sim: {embeddings.CosineSimilarity(e1.Vector, e2.Vector)}");
		}
	}
}
